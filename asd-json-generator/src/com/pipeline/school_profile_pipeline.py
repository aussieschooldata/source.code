...
def build_school_profile_sql() -> str:
    selected_columns = ",\n        ".join(
        f"source_data.{column_name}" for column_name in SCHOOL_COLUMNS if column_name != "school_url"
    )
...
def write_school_profile_files(df: pd.DataFrame, output_dir: Path) -> tuple[int, list[dict[str, Any]]]:
    if df.empty:
    ...
    sitemap_entries.sort(key=lambda item: (str(item.get("state") or ""), str(item.get("school_name") or "")))
    return files_written, sitemap_entries


def write_school_profile_sitemap_file(sitemap_entries: list[dict[str, Any]], output_path: Path) -> None:
...
def run_school_profile_pipeline() -> None:
    ...
    files_written, sitemap_entries = write_school_profile_files(df, SCHOOL_PROFILE_OUTPUT_DIR)
    write_school_profile_sitemap_file(sitemap_entries, SCHOOL_PROFILE_SITEMAP_OUTPUT_PATH)
